...
def convert_to_srt_with_punctuation(transcript):
...
def convert_to_srt_without_punctuation(transcript):
    # No sentence end pattern
    noise_tag_pattern = re.compile(r'^\s*[\[\(][^\]\)]*[\]\)]\s*$')
    MAX_CHAR_LENGTH = 120  # Drop very long cues that are hard to read
    MIN_ALPHA_RATIO = 0.45  # Require most characters to be alphanumeric
    ...
    # Merge settings - adjusted for no punctuation
    MAX_WORDS = 12  # Maximum words before forcing split
    START_LEAD_SECONDS = 0.05  # Show subtitle a bit earlier to catch the first phoneme
    END_TRIM_SECONDS = 0.05  # Trim the tail so the next cue is not blocked
    MIN_GAP_SECONDS = 0.0  # Allow back-to-back cues when we already trim the previous tail
    MIN_DURATION_SECONDS = 0.35  # Keep each subtitle shown long enough to read
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python get_youtube_srt.py <video_id> [with|without]", file=sys.stderr)
        sys.exit(1)

    video_id = sys.argv[1]
    punctuation_type = sys.argv[2] if len(sys.argv) == 3 else "with"

    if punctuation_type not in ["with", "without"]:
        print("Invalid punctuation type. Use 'with' or 'without'", file=sys.stderr)
        sys.exit(1)

    transcript = get_transcript(video_id)

    if transcript:
        if punctuation_type == "without":
            srt_text, item_count = convert_to_srt_without_punctuation(transcript)
        else:
            srt_text, item_count = convert_to_srt_with_punctuation(transcript)
        result = {
            "success": True,
            "srt": srt_text,
